    ...
    def start(self):
        if check_bd():
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
                ...
                while True:
                    read_sockets, write_sockets, error_sockets = select.select(self.__sockets_list, [],
                                                                               self.__sockets_list)
                        ...
                        try:
                            if notified_socket == server_socket:
                                ...
                                if (request := self.__get_request(notified_socket)) and request.startswith('GET'):
                                    ...
                                    self.__send_response(notified_socket, self.__create_byte_data('GET',
                                                                                                  f"{client_id}",
                                                                                                  f"{token}"))
                                    ...
                                    self.__send_response(notified_socket, self.__create_byte_data('CHECK',
                                                                                                  f"{token}",
                                                                                                  f"{client_id}"))
                                elif request.startswith('TEST'):
                                    self.__send_response(notified_socket, self.__create_byte_data('TEST_SUCCESS'))
